 ...
 * MYSQL 公用类库
 * ============================================================================
 * 版权所有 (C) 2008-2011 海南慧农电子商务有限公司，并保留所有权利。
 * 网站地址: http://www.hainanhuinong.com
 * -------------------------------------------------------
...
class cls_mysql
{
    ...
    protected $transTimes      = 0; // add by tongxuefen 2011-04-28  事务的次数
    ...
    function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
    	 ...
    	 *用口令加密数据库密码 
    	 * 
		*$dbps_con = $dbpw.DOUJIA;
		*$get = sha1($dbps_con);
		*$dbpw = substr($get,6,12);
    	 */
        define('CLIENT_MULTI_RESULTS', '131072');
        if ($pconnect)
        {
            if (!($this->_link_id = @mysql_pconnect($dbhost, $dbuser, $dbpw, 1, CLIENT_MULTI_RESULTS)))
            {
                if (!$quiet)
                {
                    $this->ErrorMsg();
        ...
        else
        {
            if (PHP_VERSION >= '4.2')
            {
                $this->_link_id = mysql_connect($dbhost, $dbuser, $dbpw, true, CLIENT_MULTI_RESULTS);
            }
            else
            {
                $this->_link_id = mysql_connect($dbhost, $dbuser, $dbpw, 1, CLIENT_MULTI_RESULTS);
    ...
    function query($sql, $type = '', $times=0)
    {
        ...
        if($type!='ejabberd'){   //if add by tongxuefen 2011-03-15 因ejabberd无表名
    ...
    function ErrorMsg($message = '', $sql = '')
    {
        ...
        else
        {
            ...
            //TODO
            //非php错误, php的错误机制捕捉不到, 暂且先这样 
            echo "<b>MySQL server error report:</b><br />";
            echo "Error:",$error, "<br />";
            echo "Errno:", $error_no, "<br />";
            echo '<a href="' . SITE_URL . '/help/faq.php?type=mysql&amp;dberrno=' . $error_no . '&amp;dberror=' . urlencode($error) . '" target="_blank">'. Lang::get('mysql_error_report') . '</a>';
        }
